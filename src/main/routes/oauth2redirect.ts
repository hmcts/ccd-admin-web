...
export const oauth2redirect = (req, res, next) => {
  ...
  console.info("[TEMP oauth2redirect] Callback received", {
  ...
  if (!expectedState || !receivedState || expectedState !== receivedState) {
    ...
    console.warn("[TEMP oauth2redirect] State validation failed", {
  ...
  console.info("[TEMP oauth2redirect] State validation succeeded");
  // Single-use nonce: consume immediately after successful validation.
  delete req.session.oauthState;

  if (!req.query.code) {
    setTempFlowStep(res, "missing-oauth2-code");
    console.warn("[TEMP oauth2redirect] Missing OAuth2 code in callback query");
    return next({
    ...
    console.info("[TEMP oauth2redirect] Exchanging authorization code for token", {
      redirectUri: req.query.redirect_uri,
    });

    accessTokenRequest(req, redirectUri)
      .then((result) => {
        console.info("[TEMP oauth2redirect] Token exchange succeeded", {
        ...
        console.info("[TEMP oauth2redirect] Access token cookie set; redirecting to home");
        // Redirect to / (index)
        res.redirect(302, "/");
      })
      .catch((err) => {
        setTempFlowStep(res, "token-exchange-failed");
        console.error("[TEMP oauth2redirect] Token exchange failed", err);
        next(err);
      });
  }
};
