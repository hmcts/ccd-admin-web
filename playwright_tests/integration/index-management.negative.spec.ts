...
test.describe("mocked index-management UI states - negative", () => {
  ...
  for (const status of FAILURE_STATUS_CODES) {
    test(`renders a Global Search HTTP ${status} response as a red error`, async ({ page }) => {
    ...
    test(`renders a case-type lookup HTTP ${status} response and allows retry`, async ({ page }) => {
      const indexManagementPage = new IndexManagementPage(page);
    ...
    test(`continues Elasticsearch processing after an HTTP ${status} case-type response`, async ({ page }) => {
      const indexManagementPage = new IndexManagementPage(page);
